    ...
    stage('Build and push Docker image') {
      steps {
        ...
        stage('Deploying APP') {
            steps {
                sshagent(['cd294e96-9019-40dc-9c01-315a49a609f3']) {
                    sh 'ssh -o StrictHostKeyChecking=no root@${ANSIBLE_SERVER} "export VERSION=${VERSION} && export DOCKER_REGISTRY=${DOCKER_REGISTRY} && export DOCKER_REPO=${DOCKER_REPO} && cd Ansible && ansible-playbook playbook.yml"'
            }
        }
